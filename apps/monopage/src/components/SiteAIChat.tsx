...
export function SiteAIChat({ username, theme }: SiteAIChatProps) {
  ...
  const widget = (
    <>
      ...
      {open && (
        <div
          className="fixed bottom-6 right-6 z-[99999] w-[340px] sm:w-[380px] flex flex-col rounded-2xl shadow-2xl overflow-hidden"
          style={{ maxHeight: '480px', backgroundColor: t?.bg || '#ffffff', border: `1px solid ${t?.cardBorder || '#e5e5e5'}` }}
        >
          {/* Header */}
          ...
          {/* Messages */}
          <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-3" style={{ minHeight: '180px', maxHeight: '320px' }}>
            {messages.length === 0 && (
  ...
  // createPortal로 body 직접 렌더링 — 부모 overflow/contain 영향 차단
