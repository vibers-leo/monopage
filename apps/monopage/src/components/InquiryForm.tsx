...
export function InquiryForm({ profileId, username, theme, ctaText, title, collapsed = true }: InquiryFormProps) {
  ...
  return (
    <div className="w-full">
      ...
      <button
        ...
        className="w-full flex items-center justify-between py-3.5 px-4 rounded-xl text-[15px] font-semibold transition-all"
        style={{
          backgroundColor: open ? 'transparent' : (t?.cardBg || '#f5f5f5'),
          border: open ? 'none' : `1px solid ${t?.cardBorder || '#e5e5e5'}`,
          color: t?.text || '#0a0a0a',
        }}
      >
        <span>{title || '문의하기'}</span>
        <ChevronDown
          size={16}
          className="transition-transform"
          style={{ transform: open ? 'rotate(180deg)' : 'rotate(0deg)', color: t?.textMuted || '#a3a3a3' }}
        />
      </button>
